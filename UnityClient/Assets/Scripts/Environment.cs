...
public class Environment : MonoBehaviour
{
    ...
    public int TileGridWidth = 16;
    public int TileGridHeight = 16;
    ...
    public void UpdateMap(Dictionary<string, object> packet)
    {
        ...
        for (int r = 0; r < mapSz; r++)
        {
            ...
            for (int c = 0; c < mapSz; c++)
            {
                foreach (KeyValuePair<int, string> entry in this.idxMats)
                {
                    string mat_name = entry.Value;
                    this.env[r, c][mat_name].SetActive(false);
                }
                int val = System.Convert.ToInt32(row[c]);
                string mat = this.idxMats[val];
                this.env[r, c][mat].SetActive(true);

                if (mat == "Forest")
                {
                    this.env[r, c]["Forest"].SetActive(true);
                    this.env[r, c]["Grass"].SetActive(true);
                }
                else if (mat == "Scrub")
                {
    ...
    private void MakeCube(GameObject chunk, int val, int R, int C, int r, int c)
    {
        //stone
        GameObject cube;
        if (val == 5)
        ...
        this.env[R + r, C + c][this.idxMats[val]] = cube;
        ...
        if (this.idxMats[val] == "Forest" || this.idxMats[val] == "Scrub")
        {
            ...
            forest.transform.eulerAngles = new Vector3(0, UnityEngine.Random.Range(0, 360), 0);
    ...
    void makeChunk(GameObject root, int sz, int R, int C)
    {
        ...
        for (int r = 0; r < sz; r++)
        {
            for (int c = 0; c < sz; c++)
            {
                ...
                //MakeCube(chunk, val, R, C, r, c);

                foreach (KeyValuePair<int, string> entry in this.idxMats)
                {
                    int terr_val = entry.Key;
                    MakeCube(chunk, terr_val, R, C, r, c);
                }
                this.env[R + r, C + c][this.idxMats[val]].SetActive(true);
                if (this.idxMats[val] == "Forest" || this.idxMats[val] == "Scrub")
                {
                    this.env[R + r, C + c]["Grass"].SetActive(true);
                }
            }
    ...
    void Update()
    {
    file:///home/sme/neural-mmo/forge/embyr/UnityClient/Assets/Scripts/Comms.cs
