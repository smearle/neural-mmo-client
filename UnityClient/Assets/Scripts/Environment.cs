...
public class Environment : MonoBehaviour
{
    ...
    public void initTerrain(Dictionary<string, object> packet)
    {
        ...
        this.orerock = Instantiate(this.orerockPrefab) as GameObject;
        this.orerockMaterial = orerock.GetComponent<MeshRenderer>().material;
        this.orerockMesh = orerock.GetComponent<MeshFilter>().sharedMesh;
        ...
        for (int r = 0; r < sz; r++)
        {
            for (int c = 0; c < sz; c++)
            {
                ...
                if (name == "Orerock")
                {
                    ...
                    this.entityManager.AddComponentData(entity, new RenderBounds { Value = orerockMesh.bounds.ToAABB() });
                    this.entityManager.AddSharedComponentData(entity, new RenderMesh { mesh = orerockMesh, material = orerockMaterial });
